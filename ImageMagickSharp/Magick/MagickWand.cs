...
namespace ImageMagickSharp
{
    ...
    public class MagickWand : WandCore<MagickWand>, IDisposable
    {
        ...
        /// <summary> Initializes a new instance of the ImageMagickSharp.MagickWand class. </summary>
        /// <exception cref="Exception"> Thrown when an exception error condition occurs. </exception>
        /// <param name="bytes"> Raw content of the image. </param>
        public MagickWand(byte[] bytes)
        {
            Wand.EnsureInitialized();
            this.Handle = MagickWandInterop.NewMagickWand();
            if (this.Handle == IntPtr.Zero)
        ...
        public bool OpenImage(byte[] bytes)
        {
            ...
            try
            {
                ...
                bool checkErrorBool = this.CheckErrorBool(ImageWandInterop.MagickReadImageBlob(this, pointer, bytes.Length));
        ...
        {
            int length;
            IntPtr pointer = ImageWandInterop.MagickGetImageBlob(this, out length);
            if (length > 0)
            {
                // Copy it
